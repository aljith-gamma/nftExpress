...
const MyNFTokens = () => {
  ...
  const { getMyNFTokenData, userData } = useStateContext();
  ...
  const fetchNFTokenData = async () => {
    const res = await getMyNFTokenData();
    // console.log({ res })
    setNFTokens(res);
  }
    ...
    <Box pt={9} px={8}>
      ...
      <Box display="grid" gridTemplateColumns={["repeat(1, 1fr)", "repeat(1, 1fr)", "repeat(2, 1fr)", "repeat(3, 1fr)"]} gap={3} mt={2}>
        {NFTokens.map((token, i) => {
          return <SingleNFToken key={i} {...token} myNft={true}/>
        })}
      </Box>
      { !NFTokens.some((token) => token.owner.toLowerCase() === userData.account) && <Typography>No NFTs!</Typography> }
    </Box>
  )
}

export default MyNFTokens